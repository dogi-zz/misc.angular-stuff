...
import {genericFormGetJsonPart} from '../../cypress/support/tools';
...
describe('generic-form-showcase-conditions', () => {
  ...
  it('Child Count Switch', () => {
    ...
    cy.get(selectorChildName).should('have.class', 'generic-form-error');
    cy.get(selectorChildAge).should('have.class', 'generic-form-error');

    // Enter Child Data
    cy.getSettled(`${selectorChildName} input`,).type('Test Child 1').blur();
    cy.getSettled(`${selectorChildAge} input`,).type('12').blur();
    cy.get(selectorChildName).should('not.have.class', 'generic-form-error');
    cy.get(selectorChildAge).should('not.have.class', 'generic-form-error');
    ...
    cy.get(selectorChildrenNames0).should('have.class', 'generic-form-error');
    cy.get(selectorChildrenNames1).should('have.class', 'generic-form-error');

    cy.getSettled(`${selectorChildrenNames0} input`,).type('Name 1').blur();
    cy.getSettled(`${selectorChildrenNames1} input`,).type('Name 2').blur();
    cy.get(selectorChildrenNames0).should('not.have.class', 'generic-form-error');
    cy.get(selectorChildrenNames1).should('not.have.class', 'generic-form-error');
